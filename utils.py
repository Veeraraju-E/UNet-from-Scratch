...
def get_loaders(train_dir, train_maskdir, val_dir, val_maskdir, batch_size, train_transform, val_transform, num_workers=4, pin_memory=True,):
    ...
    val_loader = DataLoader(
        ...
        num_workers=num_workers,
        pin_memory=pin_memory,
        shuffle=False,  # don't shuffle the validation data loader
    )
...
def check_accuracy(loader, model, device='cpu'):
    ...
    with torch.no_grad():
        for x, y in loader:
            ...
            num_correct += (predictions == y).sum()     # sum for all the pixels
            num_pixels += torch.numel(predictions)  # number of elements -> numel
            dice_score += (2 * (predictions * y).sum())/((predictions + y).sum() + 1e-8)

    print(f'Got {num_correct}/{num_pixels} viz accuracy  = {num_correct*100}/{num_pixels}:.2f')
...
def save_predictions_as_imgs(loader, model, folder="saved_images/", device="cpu"):
    ...
    for idx, (x, y) in enumerate(loader):
        ...
        with torch.no_grad():
            predictions = torch.sigmoid(model(x))
            predictions = (predictions > 0.5).float()
        torchvision.utils.save_image(predictions, f"{folder}/pred_{idx}.png")
